...
PAL also has a static validator which recognises the common pitfalls and attack version on the kernel image. Kernel image meets all the necessary requirements if the static validator assures it has no pitfalls. PAL exhibited better CFI precision compared to Android’s CFI. PAL is the first PA scheme which evaluated performance on Mac mini based on M1 chip.
